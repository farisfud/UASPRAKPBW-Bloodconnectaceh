...
<div class="layout-wrapper">
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="{{ route('permintaan-cocok') }}" class="active"><span class="nav-icon">🩸</span> Permintaan Cocok</a>
  ...
  <main class="main-content">
    <div class="page-header">
      ...
      <div style="display:flex;gap:10px;align-items:center">
        <span class="badge badge-red" style="font-size:0.8rem;padding:6px 12px;font-weight:800">{{ auth()->user()->gol_darah ?? '-' }}{{ auth()->user()->rhesus ?? '' }}</span>
    ...
    <div class="page-content">
      ...
      <div class="alert alert-success mb-20">
        ...
        <div class="alert-content">
          ...
          <div class="alert-desc">Anda terdaftar sebagai pendonor aktif dengan golongan darah <strong>{{ auth()->user()->gol_darah ?? '-' }}{{ auth()->user()->rhesus ?? '' }}</strong> di <strong>{{ auth()->user()->lokasi ?? 'Banda Aceh' }}</strong>. Permintaan di bawah cocok dengan profil donor Anda.</div>
        </div>
      </div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px">
        <div>
          ...
          <p class="text-sm text-light">Cocok berdasarkan: Golongan Darah {{ auth()->user()->gol_darah ?? '-' }}, Rhesus {{ auth()->user()->rhesus ?? '' }}, Lokasi {{ auth()->user()->lokasi ?? '-' }}</p>
        </div>
        <a href="{{ route('profil-donor') }}" class="btn btn-ghost btn-sm">👤 Ubah Profil Donor</a>
      </div>
...
<script>
  ...
  window.committedIds  = @json($committedIds);
</script>
